...
Deno.serve(async (req) => {
  ...
  try {
    // Validate cron/scheduler authentication token
    const cronToken = Deno.env.get('NOTIFICATION_TOKEN');
    const authHeader = req.headers.get('authorization');
    const providedToken = authHeader?.replace('Bearer ', '');

    if (!cronToken || providedToken !== cronToken) {
      console.warn('Unauthorized access attempt to update-completed-surgeries');
      return new Response(
    ...
    const { data: completedSurgeries, error: updateError } = await supabase
      .from('patients')
      .update({ status: 'surgery_completed' })
      .in('status', ['surgery_scheduled', 'authorized', 'pending_scheduling'])
