...
-- Drop and recreate has_role function with proper search_path (CASCADE to handle dependencies)
...
WITH CHECK (public.has_role(auth.uid(), 'secretary'::app_role) OR public.has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Secretaries and admins can update patients" ON public.patients
FOR UPDATE TO authenticated
USING (public.has_role(auth.uid(), 'secretary'::app_role) OR public.has_role(auth.uid(), 'admin'::app_role));
...
WITH CHECK (
  ...
  (public.has_role(auth.uid(), 'secretary'::app_role) OR public.has_role(auth.uid(), 'admin'::app_role))
);
...
-- Recreate policies that use is_authenticated_user
