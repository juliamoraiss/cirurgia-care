...
BEGIN
  ...
  -- (avoids DELETE permissions/RLS failures for non-admin users)
  ...
  -- If surgery_date is SET or CHANGED
  IF NEW.surgery_date IS NOT NULL AND (OLD.surgery_date IS NULL OR OLD.surgery_date != NEW.surgery_date) THEN
    ...
    -- Complete existing incomplete tasks to avoid duplicates on reschedule (instead of DELETE)
    UPDATE public.patient_tasks
    SET completed = true,
        completed_at = now(),
        completed_by = auth.uid(),
